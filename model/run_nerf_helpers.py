...
sinkhorn_loss = SamplesLoss(loss="sinkhorn", p=2, blur=.05)
# Misc
...
def compute_nerf_depth_loss(pred_depth, target_hypothesis, ground_truth_depths, gt_depth_valid, uncertainties,  train_step=None, wandb=None,  gamma=1):

    # plot histograms for target hypothesis and pred_depth (This can be commented out, it is just for illustration purposes)
    if (train_step % 500 == 0 or train_step == 1) and wandb is not None:
        plot_idx = 1
        label = ['depth prior hypothesis', 'nerf termination distances', 'ground truth depth']
        fig = plt.figure()
        c, d, _ = plt.hist([target_hypothesis[:,plot_idx,0].detach().cpu().numpy(),pred_depth[plot_idx,:].detach().cpu().numpy(), ground_truth_depths[plot_idx].detach().cpu().numpy()], bins=50, label=label, density=True)
        plt.legend(prop={'size': 10})
        if train_step % 10000 == 0:
            wandb.log({"hypotheses": wandb.Image(fig)}, commit=False)

        m1 = []
        rand_idx = np.random.choice(range(pred_depth.shape[0]),100)
        for plot_idx in rand_idx:

            # Earth Mover's Distance
            metric = wasserstein_distance(target_hypothesis[:,plot_idx,0].detach().cpu().numpy(),pred_depth[plot_idx,:].detach().cpu().numpy())   # the bigger the more different
            m1.append(metric)
        m1 = np.stack(m1)
        m1 = np.mean(m1[np.isfinite(m1)])
        wandb.log({"EMD": m1}, commit=False)

    target_hypothesis_ = target_hypothesis.permute(1,0,2).contiguous()
    sk_loss_grad = (sinkhorn_loss(target_hypothesis_,pred_depth.unsqueeze(2)) * torch.pow(1 - uncertainties,gamma)).mean()

    wandb.log({"sinkhorn": sk_loss_grad}, commit=False)
    return sk_loss_grad
